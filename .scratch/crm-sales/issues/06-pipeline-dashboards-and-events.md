...
- [ ] Pipeline-value-by-stage query endpoint (grouped by `stageId`, closed stages bucketed
      separately from in-flight ones).
- [ ] Win/loss-rate query endpoint (counts by `outcome`, over a selectable date range).
- [ ] Activity-count query endpoint (grouped by `type` and by `createdByUserId`, over a selectable
      date range).
- [ ] Dashboard screen rendering all three, with a correct empty/zeroed state for a company with
      no Deals.
- [ ] `crm.lead.qualified`, `crm.lead.disqualified`, `crm.deal.created`, `crm.deal.stage_changed`,
      `crm.deal.won`, `crm.deal.lost` declared in the manifest's `events.emits` and actually
      emitted at the right points, after commit.
- [ ] HTTP integration tests covering each dashboard query against known seeded data (including
